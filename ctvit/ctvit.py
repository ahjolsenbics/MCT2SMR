...
class CTViT(nn.Module):
    ...
    def copy_for_eval(self):
        ...
        vae_copy.eval()
        return vae_copy.to(self.device)
    ...
    def encode(
        ...
        video_shape = tuple(tokens.shape[:-1])  # [1, 20, 20, 20]

        tokens = rearrange(tokens, 'b t h w d -> (b t) (h w) d')  # [20, 400, 512]
        # device=torch.device('cuda')
        attn_bias = self.spatial_rel_pos_bias(h, w, device = self.device)  # [8, 400,400]

        tokens = self.enc_spatial_transformer(tokens, attn_bias = attn_bias, video_shape = video_shape) # [20, 400, 512]

        tokens = rearrange(tokens, '(b t) (h w) d -> b t h w d', b = b, h = h , w = w)    # [1, 20, 20, 20 512]

        # encode - temporal

        tokens = rearrange(tokens, 'b t h w d -> (b h w) t d')  # [400, 20, 512]

        tokens = self.enc_temporal_transformer(tokens, video_shape = video_shape)  # [400, 20, 512]

        tokens = rearrange(tokens, '(b h w) t d -> b t h w d', b = b, h = h, w = w)  # [1, 20, 20, 20, 512]
    ...
    def forward(
        ...
        tokens = self.to_patch_emb(video)  # [1, 20, 20, 20, 512]

        shape = tokens.shape
        *_, h, w, _ = shape

        tokens = self.encode(tokens)  # [1, 20, 20, 20, 512]

        # quantize
        tokens, packed_fhw_shape = pack([tokens], 'b * d')  # [b, 8000, 512]  将多个具有不同维度的张量根据给定的模式（pattern）打包成一个单一的张量
        vq_mask = None
        if exists(mask):  # 不经过此处
            vq_mask = self.calculate_video_token_mask(video, mask)
        tokens, indices, commit_loss = self.vq(tokens, mask=vq_mask)  # [b, 8000, 512]
        if return_only_codebook_ids:
            indices, = unpack(indices, packed_fhw_shape, 'b *')
            return indices
        tokens = rearrange(tokens, 'b (t h w) d -> b t h w d', h=h, w=w)  # [1, 20, 20, 20, 512]
