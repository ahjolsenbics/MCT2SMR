...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        diversity_lambda = opt.get('diversity_lambda', 0.5)    #重点关注这个参数
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(batch_size, bdash, 0, self.total_vocab_size).to(device) for _ in range(group_size)]
    ...
    def old_beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        beam_seq_logprobs_table = [torch.FloatTensor(self.max_seq_length, bdash, self.total_vocab_size).zero_() for _ in range(group_size)]
